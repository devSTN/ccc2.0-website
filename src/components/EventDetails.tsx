...
export default function EventDetails() {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-slate-900 to-slate-950 relative overflow-hidden scanlines">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-cyan-900/30 via-transparent to-transparent"></div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {details.map((detail, index) => (
            <div
              key={index}
              className="p-8 bg-slate-800/30 border-4 border-cyan-500/40 hover:border-cyan-400 transition-all duration-300"
            >
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-cyan-900/40 border-2 border-cyan-500/50 flex items-center justify-center flex-shrink-0">
                  <detail.icon className="w-7 h-7 text-cyan-400" />
                </div>
                <div>
                  <p className="text-xs text-cyan-600 mb-1 pixelated font-bold">{detail.label}</p>
                  <p className="text-lg font-bold text-cyan-400 pixelated whitespace-pre-line">
                    {detail.value}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-16 p-8 bg-cyan-900/20 border-4 border-cyan-500/40">
          <h3 className="text-2xl font-bold text-cyan-400 mb-4 pixelated terminal-glow">
            {'>>> WHAT_TO_EXPECT.EXE'}
          </h3>
          <ul className="space-y-3 text-cyan-300 pixelated">
